      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8 border-b border-white/10 pb-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-6">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm">
                <Link
                  ...
                  className="flex items-center gap-3 text-white hover:text-orange-400 transition-colors group"
                >
                  {videoData.artist?.profilePicture ? (
                    <div className="h-10 w-10 rounded-full overflow-hidden border-2 border-transparent group-hover:border-orange-400 transition-colors">
                      <img
                        src={videoData.artist.profilePicture}
                        alt=""
                    ...
                    <div className="h-10 w-10 rounded-full bg-gray-800 flex items-center justify-center">
                      <span className="text-xs font-bold">
                        {videoData.artist?.firstName?.[0]}
                      </span>
                    </div>
                  )}
                  <span className="font-medium text-lg">
                    {videoData.artist?.firstName} {videoData.artist?.lastName}
                  </span>
                </Link>
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2 text-gray-400 font-mono bg-white/5 px-2 py-1 rounded">
                      ...
                      <span className="text-white">
                        {Math.floor(videoData.video.duration / 60)}m{" "}
                        {videoData.video.duration % 60}s
                      </span>
                    </div>
                  </>
                )}

                {videoData.video?.isPaid && (
                  <Badge
                    variant="secondary"
            ...
            <div className="flex gap-3">{/* Future implementation */}</div>
          </div>
        </div>
          ...
          <div className="lg:col-span-2 space-y-12">
            <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-gray-900 ring-1 ring-white/10 shadow-2xl">
              <VideoPlayer
                ...
                onPlay={() => {
                  if (videoData.video?.backgroundAudioUrl)
                    setBgAudioPlaying(false);
                }}
                onPause={() => {
              ...
              <div className="flex items-center justify-between rounded-lg bg-white/5 p-3 px-4 backdrop-blur-sm border border-white/5">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    <Volume2
                      className={`h-4 w-4 ${bgAudioPlaying ? "text-primary" : "text-gray-500"}`}
                    />
                  </div>
          ...
          <div className="space-y-8">
            ...
            {videoData.video?.synopsis ? (
              <div className="bg-white/5 rounded-xl p-6 border border-white/10">
                <h3 className="text-xs font-bold tracking-widest text-secondary mb-4 uppercase">
                  Synopsis
                </h3>
                <p className="text-gray-200 leading-relaxed italic opacity-90">
                  {videoData.video.synopsis}
                </p>
              </div>
            ) : (
            ...
            <div className="bg-transparent rounded-xl border border-white/10 overflow-hidden">
              ...
              <div className="p-6 grid grid-cols-1 gap-6">
                {videoData.video?.director && (
                  <div>
                    <h4 className="text-xs text-gray-500 uppercase tracking-widest mb-1">
                      Director
                    </h4>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {videoData.video.cast.map((actor, i) => (
                        <Badge
                          key={i}
                          variant="outline"
                          className="border-white/20 text-gray-300 font-normal"
                        >
                          {actor}
                        </Badge>
                      ))}
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                    <div>
                      ...
                      <p className="font-mono text-gray-300">
                        {(videoData.video.fileSize / (1024 * 1024)).toFixed(2)}{" "}
                        MB
                      </p>
                    </div>
            ...
            {/* Production Team */}
            {videoData.video?.productionTeam &&
              videoData.video.productionTeam.length > 0 && (
                <div className="pt-4">
                  ...
                  <div className="space-y-3">
                    {videoData.video.productionTeam.map((member, i) => (
                      <div
                        key={i}
                        className="flex justify-between items-baseline p-2 rounded-lg bg-white/5 border border-white/5 hover:border-white/20 transition-colors"
                      >
        ...
        <div className="mt-24">
          <h2 className="text-2xl font-bold mb-6">
            More from {videoData.video?.artist?.firstName}
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {[1, 2, 3, 4].map((item) => (
              <div
                key={item}
                className="aspect-video bg-gray-900 rounded-lg animate-pulse opacity-50"
              ></div>
            ))}
          </div>
        </div>
