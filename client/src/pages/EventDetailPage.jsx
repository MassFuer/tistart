...
const EventDetailPage = () => {
  ...
  const fetchEvent = async () => {
    ...
    try {
      ...
      console.error(error);
      setError(
        "Failed to load event. It might have been cancelled or removed.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const isAttending =
    ...
    event?.attendees?.some((a) => {
      ...
      return (
        attendeeId && user._id && attendeeId.toString() === user._id.toString()
      );
    });
  ...
  const handleResendConfirmation = async () => {
    setIsResending(true);
    try {
      const response = await eventsAPI.resendConfirmation(id);
      setEvent(response.data.data);
      toast.success("Confirmation email resent!");
    } catch (error) {
      toast.error(
        error.response?.data?.error || "Failed to resend confirmation email",
      );
    } finally {
  ...
  const isFull =
    event.maxCapacity > 0 && event.currentAttendees >= event.maxCapacity;
  const isPast = event.endDateTime
      ...
      <div className="relative w-full h-[250px] sm:h-[300px] md:h-[400px] sm:rounded-xl overflow-hidden mb-6 sm:mb-8 shadow-lg sm:border bg-muted">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent flex flex-col justify-end p-6 md:p-10 text-white">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <Badge
              variant="secondary"
              className="text-sm px-3 py-1 font-semibold capitalize backdrop-blur-md bg-white/20 hover:bg-white/30 text-white border-none"
            >
              {event.category}
            </Badge>
          ...
          <div className="sticky top-24 space-y-6">
            ...
            <Card className="border-0 bg-muted/30 shadow-xl z-20">
              ...
              <CardContent className="space-y-6">
                ...
                {event.maxCapacity > 0 && (
                  <div className="space-y-1">
                    <Progress
                      value={progress}
                      className="h-2 bg-muted dark:bg-zinc-700"
                    />
                    ...
                    <div className="space-y-3">
                      {(() => {
                        ...
                        const isNotConfirmed =
                          attendee?.status === "notConfirmed";
                        const isExpired =
                          isNotConfirmed &&
                          (!attendee?.confirmationTokenExpires ||
                            new Date(attendee.confirmationTokenExpires) <
                              new Date());
                        ...
                        if (isNotConfirmed) {
                          return (
                            <div className="space-y-3">
                              <div
                                className={`p-3 rounded-md flex flex-col gap-2 font-medium border ${isExpired ? "bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-300 border-red-200 dark:border-red-900" : "bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-900"}`}
                              >
                                ...
                                {isExpired && (
                                  <Button
                                    variant="link"
                                    size="sm"
                                    className="h-auto p-0 text-red-700 dark:text-red-300 underline"
                        ...
                        return (
                          <div className="bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-300 p-3 rounded-md flex items-center justify-center gap-2 font-medium border border-green-200 dark:border-green-900">
                            <CheckCircle2 className="h-5 w-5" /> You are
                            registered
                          </div>
                        );
                      })()}
                      ...
                      {event.price === 0 ? (
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                This action cannot be undone. You will lose your
                                spot in the event.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                        ...
                        <Button
                          ...
                          onClick={() =>
                            toast.info(
                              "For refunds, please contact the organizer directly or check the refund policy below.",
                            )
                          }
                        >
                          Request Refund
              ...
              <CardFooter className="bg-muted/50 p-4 text-xs text-muted-foreground text-center">
                Typically replies within a few hours. Refunds available up to
                24h before event.
              </CardFooter>
            </Card>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          ...
                          delete the event and remove all attendee data.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                ...
                <>
                  <Button
                    ...
                    className={`flex-1 ${(() => {
                      ...
                      if (isExpired)
                        return "text-red-600 border-red-200 dark:border-red-900";
                      if (isNotConfirmed) return "text-yellow-600";
                    ...
                    disabled={(() => {
                      ...
                      const isExpired =
                        ...
                        (!attendee?.confirmationTokenExpires ||
                          new Date(attendee.confirmationTokenExpires) <
                            new Date());
                      // Only clickable to resend if expired
                      return !isExpired;
                    })()}
                    onClick={(() => {
                      const attendee = event.attendees?.find(
                      ...
                      const isExpired =
                        ...
                        (!attendee?.confirmationTokenExpires ||
                          new Date(attendee.confirmationTokenExpires) <
                            new Date());
                      return isExpired ? handleResendConfirmation : undefined;
                    })()}
                  >
                    {(() => {
                      const attendee = event.attendees?.find(
                      ...
                      if (isExpired)
                        return isResending ? "Resending..." : "Resend Link";
                      if (isNotConfirmed) return "Pending";
                      return "Registered";
                    })()}
                  </Button>
                    ...
                    <div className="flex-shrink-0">
                      {event.price === 0 ? (
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                You will lose your spot for this event. This
                        ...
                        <Button
                          ...
                          onClick={() =>
                            toast.info(
                              "For refunds, please contact the organizer directly or check the refund policy.",
                            )
                          }
                        >
