...
const AttendeesModal = ({ eventId, open, onOpenChange }) => {
  ...
  const handleMessageAttendee = async (attendeeUser) => {
    try {
      const response = await messagingAPI.createConversation({
        participantId: attendeeUser._id,
      });
              ...
              return (
                ...
                >
                  <Avatar className="h-10 w-10">
                    <AvatarImage
                      src={attendeeUser.avatar || attendeeUser.profilePicture}
                    />
                    <AvatarFallback>
                      {attendeeUser.firstName?.[0] ||
                        attendeeUser.userName?.[0] ||
                        "?"}
                    </AvatarFallback>
                  </Avatar>
