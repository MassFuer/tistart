...
const EventCard = ({ event, showActions = false, onDelete }) => {
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden hover:shadow-md transition-shadow">
      <Link
        to={`/events/${event._id}`}
        className="block relative aspect-video w-full overflow-hidden bg-muted group cursor-pointer"
      >
        {event.image ? (
          <img
            src={event.image}
        ...
        <div className="absolute top-2 right-2 flex gap-2">
          <Badge
            variant="secondary"
            className="capitalize backdrop-blur-md bg-white/80 dark:bg-black/60"
          >
            {event.category}
          </Badge>
        </div>
      ...
      <CardFooter className="p-2.5 md:p-4 flex gap-1.5 md:gap-2 border-t bg-muted/20 mt-auto">
        {showActions ? (
          <>
            ...
            >
              <Link to={`/events/${event._id}/edit`}>
                <Edit className="h-3 w-3 md:mr-1.5" />{" "}
          ...
          <>
            ...
            <Button
              size="sm"
              className="flex-1 h-8 text-xs dark:bg-gray-200 dark:text-black dark:hover:bg-gray-200/90 bg-primary text-primary-foreground"
              asChild
              disabled={isFull}
            >
              <Link to={`/events/${event._id}`}>
                <Ticket className="h-3 w-3 md:mr-1.5" />{" "}
