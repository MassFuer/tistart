...
const VideoHero = ({ compact = false }) => {
  ...
  return (
    <div
      className={`relative w-full bg-black ${compact ? "h-full" : "h-[150vh]"}`}
    >
      {/* Sticky Container for the video effect */}
      <div
        className={`${compact ? "absolute inset-0 h-full" : "sticky top-0 h-screen"} w-full overflow-hidden flex items-center justify-center @container`}
      >
        {/* Background Video */}
        ...
        {/* Text Overlay */}
        <div className="relative z-20 px-4 sm:px-8 w-full flex flex-col items-center justify-center text-center pointer-events-none py-12">
          <h1
            className="font-black uppercase leading-[0.8] tracking-tighter text-white/50 mix-blend-overlay drop-shadow-2xl select-none italic"
            style={{
              fontSize: heroConfig.textSize || "clamp(2.5rem, 11cqi, 18rem)",
            }}
          >
            {heroText === "VIDEO ARTWORKS by NEMESIS ART" ? (
              <div className="flex flex-col items-center">
                <span className="whitespace-nowrap">VIDEO ARTWORKS</span>
                <span className="text-[0.45em] opacity-80 tracking-[0.2em] font-light not-italic mt-[0.2em] whitespace-nowrap">
                  by NEMESIS ART
                </span>
              </div>
            ) : (
              <span className="break-words">{heroText}</span>
            )}
          </h1>
        </div>
        {/* Controls */}
        <div className="absolute bottom-10 left-0 right-0 z-30 flex items-center justify-between px-8 sm:px-12 pointer-events-auto">
          ...
          <Button
            ...
            variant="ghost"
            size="md"
            className="rounded-full bg-white/10 text-white backdrop-blur-sm hover:bg-white/30 hover:text-white px-6 h-10 gap-3 text-md"
          >
