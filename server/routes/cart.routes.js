    ...
    try {
      ...
      // --- HANDLE EVENT (TICKET) ---
        ...
        if (event.price === 0) {
          return res
            .status(400)
            .json({ error: "Free events do not require a ticket purchase. Please join directly." });
        }
        ...
        if (existingItemIndex > -1) {
          ...
          if (newQuantity > 3) {
            return res
              .status(400)
              .json({ error: "You can only purchase a maximum of 3 tickets per event." });
          }
          user.cart[existingItemIndex].quantity = newQuantity;
        } else {
