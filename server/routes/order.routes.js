    ...
    for (const item of user.cart) {
      ...
      if (item.itemType === "artwork") {
        ...
        if (!artwork.isForSale) {
          await rollbackStocks(stockUpdates);
          return res.status(400).json({ error: `"${artwork.title}" is no longer available for sale.` });
        }
      ...
      // --- HANDLE TICKET ---
          ...
          if (!event) {
              await rollbackStocks(stockUpdates);
              return res.status(400).json({ error: "One or more events in cart are no longer available." });
          }
          ...
          const userTicketCount = event.attendees ? event.attendees.filter(a => a.user && a.user.toString() === req.payload._id).length : 0;
          if (userTicketCount + item.quantity > 3) {
              await rollbackStocks(stockUpdates);
              return res.status(400).json({ error: `You can only hold a maximum of 3 tickets for "${event.title}". You already have ${userTicketCount}.` });
          }
          ...
          if (event.maxCapacity > 0) {
               query.$expr = { $lte: [{ $add: [{ $size: "$attendees" }, item.quantity] }, event.maxCapacity] };
          }
          ...
          if (!updatedEvent) {
               await rollbackStocks(stockUpdates);
               return res.status(400).json({ error: `Checking out tickets for "${event.title}" failed. Event may be full.` });
          }
